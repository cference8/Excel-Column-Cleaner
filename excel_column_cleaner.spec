...
a = Analysis(
    ['excel_column_cleaner.py'],
    pathex=['.'],  # Specify the path if additional imports are in subdirectories
    binaries=[('poppler-24.08.0/Library/bin', 'poppler')],  # poppler binaries path
    datas=[('scribe-logo-final.png', '.'), ('scribe-icon.ico', '.')],  # data files like logo and icon
    hiddenimports=['pdf2image', 'PIL', 'PyPDF2'],  # include dependencies explicitly
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[],
    excludes=[],
